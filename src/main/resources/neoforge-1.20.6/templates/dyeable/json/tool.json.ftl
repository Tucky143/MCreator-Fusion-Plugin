{
    ...
    "textures": {
        <#if var_item??>
            ...
            <#if data.getDyeablePart(var_item).hasOverlayTexture()>,
            "layer1": "${data.getItemOverlayTextureFor(var_item).format("%s:item/%s")}"
            </#if>
        <#else>
            "layer0": "${data.texture.format("%s:item/%s")}"

            <#if data.hasOverlayTexture()>,
            "layer1": "${data.overlayTexture.format("%s:item/%s")}"
            </#if>
        </#if>
    }
