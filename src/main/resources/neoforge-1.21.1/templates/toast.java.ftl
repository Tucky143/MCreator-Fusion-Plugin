...
public class ${name}Toast implements Toast {
    ...
    public Visibility render(GuiGraphics guiGraphics, ToastComponent component, long lastChanged) {
         guiGraphics.blit(<#if data.background == "Default">TEXTURE<#else>ResourceLocation.parse("${modid}:textures/screens/${data.background}")</#if>, 0, 0, 0, 32, this.width(), this.height());
		...
		<#if data.icon != "None">
         guiGraphics.blit(ResourceLocation.parse("${modid}:textures/screens/${data.icon}"), 6, 6, 296, 120, 20, 20);
		</#if>
